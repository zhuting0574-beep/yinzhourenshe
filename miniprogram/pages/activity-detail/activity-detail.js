const { request, requireLogin, scanActivity } = require('../../utils/request')
Page({
  data: { item: null, submitting: false },
  onLoad(o) {
    this.load(o.id)
  },
  load(id) {
    return request(`/mini/activities/${id}`).then((item) => this.setData({ item }))
  },
  back() {
  ...
  register() {
    ...
    this.setData({ submitting: true })
    requireLogin()
      .then(() => request(`/mini/activities/${item.id}/register`, { method: 'POST' }))
      .then((updated) => {
  ...
  locationCheckIn() {
    ...
    requireLogin()
      ...
      .catch((error) => {
        if (error && error.errMsg && String(error.errMsg).includes('getLocation'))
          wx.showModal({
            title: '无法获取位置',
            content:
