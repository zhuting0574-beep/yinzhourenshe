...
Page({
  ...
  initialProgress(total: number, today: number) {
    return Math.min(1, Math.max(0, (today - 2) / Math.max(1, total - 7)))
  },
  scrollCalendar(e: any) {
  ...
  load() {
    request<any>('/mini/check-ins').then((d) => {
      const days = d.days.map((x: any) => String(x.check_date).slice(8, 10))
      const now = new Date()
    ...
    request<any>('/mini/profile').then((u) => this.setData({ points: u.points }))
  },
  submit() {
