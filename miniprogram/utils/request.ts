...
export function request<T>(url: string, options: Wechat.RequestOption = {}) {
  return new Promise<T>((resolve, reject) =>
    wx.request({
      ...
      success: (r) => {
        const body = r.data as ApiResult<T>
...
export function requireLogin(route?: string) {
...
export function login() {
  return new Promise((resolve, reject) =>
    wx.login({
      success: ({ code }) =>
        request<any>('/mini/auth/wechat', { method: 'POST', data: { code } })
          .catch(() =>
            request<any>('/dev/auth/mock', {
              method: 'POST',
...
export function bindPhone(code: string) {
  return request<any>('/mini/auth/phone', { method: 'POST', data: { code } }).then((result) => {
    app.globalData.token = result.token
    app.globalData.user = result.user
    wx.setStorageSync('token', result.token)
    wx.setStorageSync('user', result.user)
